...
public class KonkretnyNPCStatyczny : NPCClass, ICzekajAz
{
    ...
    */
        ...
        if (this.aktualnyReuseAtaku < szybkośćAtaku)
        {
            ...
            if (f <= .15f)
            {
                if (!instaObjIsActive)
                {
                    ...
                    if (SprawdźCzyWidocznaPozycja() && efektyFxStart != null)
                    {
        ...
        else  //
        {
            ...
            switch (typAtakuWieży)
            {
                case TypAtakuWieży.jedenTarget: //Jeden Target
                    cel.ZmianaHP((short)(Mathf.CeilToInt(zadawaneObrażenia * modyfikatorZadawanychObrażeń) + ManagerGryScript.bonusDoObrażeń));
                    if (cel.NieŻyję)
                    {
                ...
                case TypAtakuWieży.wybuch: //Wybuch
                    ...
                    for (byte i = 0; i < iloscCol; i++)
                    {
                        ...
                        klasa.ZmianaHP((short)(Mathf.CeilToInt(zadawaneObrażenia * modyfikatorZadawanychObrażeń) + ManagerGryScript.bonusDoObrażeń));
                    }
                    if (cel.NieŻyję)
                    {
                        //Debug.Log("No to jedziemy");
                ...
                case TypAtakuWieży.wszyscyWZasiegu: //Wszystkie cele
                    if (rootEnemy != null)
                    {
                        ...
                        {
                            wrogowieWZasiegu[i].ZmianaHP((short)(Mathf.CeilToInt(zadawaneObrażenia * modyfikatorZadawanychObrażeń) + ManagerGryScript.bonusDoObrażeń));
                        }
                        /*
                        if (cel.NieŻyję)
                        {
    ...
    public void Napraw(bool payForRepair = true)
    {
        if (payForRepair)
        {
            PomocniczeFunkcje.managerGryScript.UstawIlośćCoinów((short)-kosztNaprawy);
    ...
    public void UpgradeMe(int whatUpgrade)
    {
        ...
        {
            case 0: //HP
                this.maksymalneŻycie = (short)(this.bazoweHP + 2 * PomocniczeFunkcje.managerGryScript.hpIdx);
                RysujHPBar();
                break;
            case 1: //Attack
                this.modyfikatorZadawanychObrażeń = PomocniczeFunkcje.WyliczModyfikatorObrazeń(this.bazowyAtak, PomocniczeFunkcje.managerGryScript.atkIdx);
                break;
            case 2: //Defence
